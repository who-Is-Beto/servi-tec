...
<template>
  <UiSection id="agendar" class="bg-paper-mute dark:bg-ink-950">
    ...
    <div class="mt-12 grid items-start gap-10 lg:grid-cols-[1fr_1.1fr]">
      ...
      <div>
        <ul class="space-y-3">
          <li
            ...
            :key="b.titulo"
            v-reveal="{ delay: i * 80 }"
            class="flex items-start gap-4 rounded-2xl bg-white p-5 ring-1 ring-ink-200/60 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-card hover:ring-brand-200 dark:bg-ink-900 dark:ring-ink-800"
          >
            <span
        ...
        >
          <span
            aria-hidden="true"
            class="pointer-events-none absolute -right-16 -top-16 size-48 rounded-full bg-brand-600/25 opacity-70 blur-3xl transition-opacity duration-300 group-hover:opacity-100 dark:bg-brand-500/25"
          />
      ...
      <!-- Formulario -->
      <div v-reveal="{ from: 'right' }" class="rounded-3xl bg-white p-6 shadow-card ring-1 ring-ink-200/60 sm:p-8 dark:bg-ink-900 dark:ring-ink-800">
        <ScheduleForm />
      </div>
    </div>
  </UiSection>
